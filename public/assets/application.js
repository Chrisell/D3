...
(function() {
  $(document).ready(function() {
    if ($('.bars').length > 0) {
      return $.ajax('orders.json', {
        success: function(data, status, xhr) {
          var bodySelection, color, layer, nested_data, orders_by_day, rect, stack, states, svg, svgHeight, svgWidth, transitionGrouped, transitionStacked, x, y, yGroupMax, yStackMax;
          ...
          orders_by_day = stack(d3.range(states).map(function(d) {
            return nested_data[d].values;
          }));
          yGroupMax = d3.max(orders_by_day, function(layer) {
            return d3.max(layer, function(d) {
              return d.values.length;
            });
          });
          yStackMax = d3.max(orders_by_day, function(layer) {
            return d3.max(layer, function(d, i) {
              return d.values.length;
            });
          });
          console.log('yGroupMax: ' + yGroupMax + ', yStackMax: ' + yStackMax);
          x = d3.scale.ordinal().domain(d3.range(31)).rangeRoundBands([0, svgWidth], .08);
          y = d3.scale.linear().domain([0, yGroupMax]).range([svgHeight, 0]);
          bodySelection = d3.select("body");
          svg = bodySelection.append("svg").attr("width", svgWidth).attr("height", svgHeight);
          layer = svg.selectAll(".layer").data(orders_by_day).enter().append("g").attr("class", "layer").style("fill", function(d, i) {
            return color(i);
          }).style('opacity', 0.5);
          rect = layer.selectAll("rect").data(function(d) {
            ...
            return x(i);
          }).attr("y", svgHeight).attr("width", 20).attr("height", 0);
          transitionStacked = function() {
            y.domain([0, yStackMax]);
            return rect.transition().duration(500).delay(function(d, i) {
              return i * 10;
            }).attr("y", function(d) {
          ...
          transitionGrouped = function() {
            ...
            return rect.transition().delay(function(d, i) {
              ...
              return x(i) + x.rangeBand() / states * j;
            }).attr("width", x.rangeBand() / states).transition().attr("y", function(d) {
              return y(d.values.length);
            }).attr("height", function(d) {
          ...
          return $('input').change(function() {
            if ($(this).val() === "grouped") {
              ...
              return transitionGrouped();
            } else {
              return transitionStacked();
            }
          });
        },
...
(function() {
  $(function() {
    var bodySelection, circleAttributes, circles, collide, colors, force, h, nodes, root, svg, w;
    ...
    if ($('.inputs').length > 0) {
      $('#one').keyup(function() {
        ...
        return circles.transition().attr('r', value);
      });
      ...
      h = 600;
      bodySelection = d3.select("body");
      svg = bodySelection.append("svg:svg").attr("width", w).attr("height", h);
      circles = svg.selectAll("circle").data(nodes.slice(1)).enter().append("circle");
      circleAttributes = circles.attr("r", function(d) {
        return d.radius - 2;
      }).style("fill", 'red');
