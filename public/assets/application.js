...
(function() {
  $(document).ready(function() {
    return $.ajax('data.json', {
      success: function(data, status, xhr) {
        var bodySelection, circleAttributes, circles, color, gasData, svgSelection;

        color = d3.rgb(0, 0, 0);
        bodySelection = d3.select("body");
        svgSelection = bodySelection.append("svg").attr("width", 1200).attr("height", 800);
        gasData = data.dates;
        circles = svgSelection.selectAll("circle").data(gasData).enter().append("circle");
        circleAttributes = circles.attr("cx", function(d, index) {
          return index * 2;
        }).attr("cy", function(d) {
          return 800;
        }).attr("r", function(d) {
